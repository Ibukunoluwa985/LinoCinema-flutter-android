...
class CrimeCategory extends StatefulWidget {
  CrimeCategory(this.movies, this.tvshows);
  ...
  @override
  _CrimeCategoryState createState() => _CrimeCategoryState();
}
...
class _CrimeCategoryState extends State<CrimeCategory> {
  getMovieGenre() {
    ...
    for (var i = 0; i < widget.movies.length; i++)
      movie = widget.movies
          .where((m) => m['genre'].toString() == 'Crime')
          .toList();
    for (var i = 0; i < widget.movies.length; i++) {
      if (widget.movies[i]['genre'].toString() == 'Crime') {
  ...
  getTvshowGenre() {
    ...
    for (var i = 0; i < widget.tvshows.length; i++)
      tvshow = widget.tvshows
          .where((t) => t['genre'].toString() == 'Crime')
          .toList();
    for (var i = 0; i < widget.tvshows.length; i++) {
      if (widget.tvshows[i]['genre'].toString() == 'Crime') {
        return GridView.builder(
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            crossAxisCount: 3,
            childAspectRatio: (((MediaQuery.of(context).size.height) / 7) /
