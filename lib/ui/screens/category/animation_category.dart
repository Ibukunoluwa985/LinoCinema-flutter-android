...
class AnimationCategory extends StatefulWidget {
  AnimationCategory(this.movies, this.tvshows);
  ...
  @override
  _AnimationCategoryState createState() => _AnimationCategoryState();
}

class _AnimationCategoryState extends State<AnimationCategory> {
  getMovieGenre() {
    var movie;
    for (var i = 0; i < widget.movies.length; i++)
      movie = widget.movies
          .where((m) => m['genre'].toString() == 'Animation')
          .toList();
    for (var i = 0; i < widget.movies.length; i++) {
      if (widget.movies[i]['genre'].toString() == 'Animation') {
  ...
  getTvshowGenre() {
    ...
    for (var i = 0; i < widget.tvshows.length; i++)
      tvshow = widget.tvshows
          .where((t) => t['genre'].toString() == 'Animation')
          .toList();
    for (var i = 0; i < widget.tvshows.length; i++) {
      if (widget.tvshows[i]['genre'].toString() == 'Animation') {
        return GridView.builder(
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            crossAxisCount: 3,
            childAspectRatio: (((MediaQuery.of(context).size.height) / 7) /
