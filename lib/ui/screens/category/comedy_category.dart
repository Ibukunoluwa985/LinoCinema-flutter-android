...
class ComedyCategory extends StatefulWidget {
  ComedyCategory(this.movies, this.tvshows);
  ...
  @override
  _ComedyCategoryState createState() => _ComedyCategoryState();
}

class _ComedyCategoryState extends State<ComedyCategory> {
  getMovieGenre() {
    var movie;
    for (var i = 0; i < widget.movies.length; i++)
      movie = widget.movies
          .where((m) => m['genre'].toString() == 'Comedy')
          .toList();
    for (var i = 0; i < widget.movies.length; i++) {
      if (widget.movies[i]['genre'].toString() == 'Comedy') {
  ...
  getTvshowGenre() {
    ...
    for (var i = 0; i < widget.tvshows.length; i++)
      tvshow = widget.tvshows
          .where((t) => t['genre'].toString() == 'Comedy')
          .toList();
    for (var i = 0; i < widget.tvshows.length; i++) {
      if (widget.tvshows[i]['genre'].toString() == 'Comedy') {
        return GridView.builder(
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            crossAxisCount: 3,
            childAspectRatio: (((MediaQuery.of(context).size.height) / 7) /
