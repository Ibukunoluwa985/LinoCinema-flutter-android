...
class DocumentaryCategory extends StatefulWidget {
  DocumentaryCategory(this.movies, this.tvshows);
  ...
  @override
  _DocumentaryCategoryState createState() => _DocumentaryCategoryState();
}

class _DocumentaryCategoryState extends State<DocumentaryCategory> {
  getMovieGenre() {
    var movie;
    for (var i = 0; i < widget.movies.length; i++)
      movie = widget.movies
          .where((m) => m['genre'].toString() == 'Documentary')
          .toList();
    for (var i = 0; i < widget.movies.length; i++) {
      if (widget.movies[i]['genre'].toString() == 'Documentary') {
  ...
  getTvshowGenre() {
    ...
    for (var i = 0; i < widget.tvshows.length; i++)
      tvshow = widget.tvshows
          .where((t) => t['genre'].toString() == 'Documentary')
          .toList();
    for (var i = 0; i < widget.tvshows.length; i++) {
      if (widget.tvshows[i]['genre'].toString() == 'Documentary') {
        return GridView.builder(
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            crossAxisCount: 3,
            childAspectRatio: (((MediaQuery.of(context).size.height) / 7) /
