...
class SciFiCategory extends StatefulWidget {
  SciFiCategory(this.movies, this.tvshows);
  ...
  @override
  _SciFiCategoryState createState() => _SciFiCategoryState();
}

class _SciFiCategoryState extends State<SciFiCategory> {
  getMovieGenre() {
    var movie;
    for (var i = 0; i < widget.movies.length; i++)
      movie = widget.movies
          .where((m) => m['genre'].toString() == 'Sci-fi')
          .toList();
    for (var i = 0; i < widget.movies.length; i++) {
      if (widget.movies[i]['genre'].toString() == 'Sci-fi') {
  ...
  getTvshowGenre() {
    ...
    for (var i = 0; i < widget.tvshows.length; i++)
      tvshow = widget.tvshows
          .where((t) => t['genre'].toString() == 'Sci-fi')
          .toList();
    for (var i = 0; i < widget.tvshows.length; i++) {
      if (widget.tvshows[i]['genre'].toString() == 'Sci-fi') {
        return GridView.builder(
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            crossAxisCount: 3,
            childAspectRatio: (((MediaQuery.of(context).size.height) / 7) /
